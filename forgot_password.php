<?php
include "show.php";
include "password_reset_code.php";
session_start();
if(isset($_POST['submit'])){
    $email=$_POST['email'];
    $sql=mysqli_query($conn,"SELECT *from `authentication` WHERE email='$email'");
    if(mysqli_num_rows($sql)==1){
        $otp=rand(1111,9999);
        $subject="PASSWORD RESET";
        send_otp($email,$subject,$otp);
        mysqli_query($conn,"UPDATE `authentication` SET otp='$otp' WHERE email='$email' ");
        $_SESSION['email']=$email;
        $_SESSION['otp']=$otp;
        header("location:verify.php");
    }
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="home_style.css">
    <style>
        .container{
    border: 1px solid rgba(170, 155, 155, 0.39);;
    border-radius: 10px;
    position: relative;
    top: 10%;
    left: 5%;
}
.title{
    font-size: 35px;
    
}
.click{
    font-size: 20px;
    border-radius: 10px;
    width: 100%;
    padding: 3px;
    border-color: rgba(170, 155, 155, 0.39);
    margin-bottom: 10px;
}
    </style>
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.2/dist/umd/popper.min.js" integrity="sha384-IQsoLXl5PILFhosVNubq5LC7Qb9DXgDA9i+tQ8Zj3iwWAwPtgFTxbJ8NT4GN1R8p" crossorigin="anonymous"></script>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
</head>
<body>
    <div class="container">
		<div class="title">Forgot Password</div>
		<form action="forgot_password.php" method="post" class="form">
				<div class="mb-3">
                <label for="">Email address</label>
  				<input class="form-control" type="email" id="formFileMultiple" name="email">
				</div>
			<button type="submit" class="click" name="submit">Click here</button>
		</form>
	</div>
</body>
</html>