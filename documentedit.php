<?php
session_start();
$update=$_REQUEST['update_task'];
$_SESSION['update_task']=$update;
if(!isset($_SESSION['loggedin']) || $_SESSION['loggedin']!=true){
    header("location:authentication_login.php");
}
include "show.php";
$result=mysqli_query($conn,"SELECT *FROM `job_form_application` where `id`='$update'");
$edit=mysqli_fetch_array($result);
$organization_description=$edit["organization_description"];
$about_job=$edit["about_job"];
$skill_required=$edit["skill_required"];
$who_can_apply=$edit["who_can_apply"];
$salary=$edit["salary"];
$additional_information=$edit["additional_information"];
$state=$edit["state"];
$city=$edit["city"];
?>
...
<form action="edit.php" method="post" class="form">
    ...
    <input type="text" class="form-control" value="<?= $edit['organization_name']?>"  name="organization_name" aria-label="Username" aria-describedby="basic-addon1">
    </div>

    <div class="input-group mb-3">
    <span class="input-group-text" id="basic-addon1">Job Post</span>
    <input type="text" class="form-control" name="job_post_place" value="<?= $edit['job_post_place']?>" aria-label="Username" aria-describedby="basic-addon1">
    </div>

    <div class="input-group mb-3">
    <span class="input-group-text" id="basic-addon2">Organization Describtion</span>
    <textarea class="form-control" aria-label="With textarea"  name="organization_description"><?php echo htmlspecialchars(stripslashes($organization_description)); ?></textarea>  
  </div>

    <div class="input-group mb-3">
    <span class="input-group-text" id="basic-addon3">About Job</span>
    <textarea class="form-control" aria-label="With textarea" name="about_job"><?php echo htmlspecialchars(stripslashes($about_job)); ?></textarea>  
  </div>

    <div class="input-group mb-3">
    <span class="input-group-text">Skill Required</span>
    <textarea class="form-control" aria-label="With textarea" name="skill_required"><?php echo htmlspecialchars(stripslashes($skill_required)); ?></textarea> 
    </div>

    <div class="input-group mb-3">
    <span class="input-group-text" id="basic-addon2">Who can apply</span>
    <textarea class="form-control" aria-label="With textarea" name="who_can_apply"><?php echo htmlspecialchars(stripslashes($who_can_apply)); ?></textarea> 
    </div>

    <div class="input-group mb-3">
    <span class="input-group-text" id="basic-addon3">Salary</span>
    <textarea class="form-control" aria-label="With textarea"  name="salary"><?php echo htmlspecialchars(stripslashes($salary)); ?></textarea> 
    </div>

    <div class="input-group mb-3">
    <span class="input-group-text">Additional Information</span>
    <textarea class="form-control" aria-label="With textarea" name="additional_information"><?php echo htmlspecialchars(stripslashes($additional_information)); ?></textarea>
    </div>

    <div class="input-group mb-3">
    <span class="input-group-text">Organization Email</span>
    <input type="text" class="form-control" name="email" value="<?= $edit['email'];?>" aria-label="Username" aria-describedby="basic-addon1">
    </div>
    
    <div class="input-group mb-3">
    <span class="input-group-text" id="basic-addon2">Contact Number</span>
    <input type="number" class="form-control" name="phone" value="<?= $edit['phone']?>" aria-label="Username" aria-describedby="basic-addon1">
    </div>
  ...
  <span class="input-group-text">Choose City</span>
    <select id="citySelect" size="1" class="form-control" value="<?= $edit['city']?>" name="city" >
